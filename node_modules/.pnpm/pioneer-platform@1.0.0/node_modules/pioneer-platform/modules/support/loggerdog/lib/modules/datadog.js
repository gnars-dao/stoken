"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g;
    return g = { next: verb(0), "throw": verb(1), "return": verb(2) }, typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (_) try {
            if (f = 1, y && (t = op[0] & 2 ? y["return"] : op[0] ? y["throw"] || ((t = y["return"]) && t.call(y), 0) : y.next) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [op[0] & 2, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
/*
    Logger-dog
        A data-dog logger module
          powered by redis


              _=,_
           o_/6 /#\
           \__ |##/
            ='|--\
              /   #'-.
              \#|_   _'-. /
               |/ \_( # |"
               C/ ,--___/

                     -Highlander

    Notes:

 */
var TAG = " index ";
var os = require('os');
var rp = require("request-promise");
var DATADOG_API_KEY = process.env["DATADOG_API_KEY"];
module.exports = {
    debug: function (tag, message, object) {
        return send_log_info(tag, message, object);
    },
    info: function (tag, message, object) {
        return send_log_info(tag, message, object);
    },
    warn: function (tag, message, object) {
        return send_log_warn(tag, message, object);
    },
    error: function (tag, message, object) {
        return send_log_error(tag, message, object);
    }
};
var send_log_info = function (tag, message, object) {
    return __awaiter(this, void 0, void 0, function () {
        var body, url, result, e_1;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    _a.trys.push([0, 2, , 3]);
                    //log to console
                    console.log(tag, message, object);
                    if (object) {
                        message = message + " object: " + JSON.stringify(object);
                    }
                    body = {
                        level: "info",
                        message: message,
                        ddsource: "testbox",
                        ddtags: "status:info,env:production,tag:" + tag,
                        user: "bro",
                        hostname: os.hostname()
                    };
                    url = "https://http-intake.logs.datadoghq.com/v1/input/" + DATADOG_API_KEY;
                    return [4 /*yield*/, sendPostRequest(url, body)];
                case 1:
                    result = _a.sent();
                    return [2 /*return*/, result];
                case 2:
                    e_1 = _a.sent();
                    console.error({ e: e_1.toString() });
                    return [3 /*break*/, 3];
                case 3: return [2 /*return*/];
            }
        });
    });
};
var send_log_warn = function (tag, message, object) {
    return __awaiter(this, void 0, void 0, function () {
        var body, url, result, e_2;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    _a.trys.push([0, 2, , 3]);
                    body = {
                        level: "warn",
                        message: message,
                        ddsource: "testbox",
                        ddtags: "status:warn,env:production",
                        user: "bro",
                        hostname: os.hostname()
                    };
                    url = "https://http-intake.logs.datadoghq.com/v1/input/" + DATADOG_API_KEY;
                    return [4 /*yield*/, sendPostRequest(url, body)];
                case 1:
                    result = _a.sent();
                    return [2 /*return*/, result];
                case 2:
                    e_2 = _a.sent();
                    console.error({ e: e_2.toString() });
                    return [3 /*break*/, 3];
                case 3: return [2 /*return*/];
            }
        });
    });
};
var send_log_error = function (tag, message, object) {
    return __awaiter(this, void 0, void 0, function () {
        var body, url, result, e_3;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    _a.trys.push([0, 2, , 3]);
                    body = {
                        level: "error",
                        message: message,
                        ddsource: "testbox",
                        ddtags: "status:error,env:production,level:error",
                        user: "bro",
                        hostname: os.hostname()
                    };
                    url = "https://http-intake.logs.datadoghq.com/v1/input/" + DATADOG_API_KEY;
                    return [4 /*yield*/, sendPostRequest(url, body)];
                case 1:
                    result = _a.sent();
                    return [2 /*return*/, result];
                case 2:
                    e_3 = _a.sent();
                    console.error({ e: e_3.toString() });
                    return [3 /*break*/, 3];
                case 3: return [2 /*return*/];
            }
        });
    });
};
var sendPostRequest = function (url, body) {
    return __awaiter(this, void 0, void 0, function () {
        var tag, options, result, e_4, errorCode, errorMessage;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    tag = TAG + " | send_post_request | ";
                    _a.label = 1;
                case 1:
                    _a.trys.push([1, 3, , 4]);
                    options = {
                        method: 'POST',
                        uri: url,
                        body: body,
                        json: true // Automatically stratifies the body to JSON
                    };
                    return [4 /*yield*/, rp(options)
                        //console.log(tag,result)
                    ];
                case 2:
                    result = _a.sent();
                    //console.log(tag,result)
                    return [2 /*return*/, result];
                case 3:
                    e_4 = _a.sent();
                    errorCode = e_4.message.replace(' - ""', '');
                    errorCode = errorCode.split("-");
                    console.error(tag, "errorCode: ", errorCode);
                    errorMessage = e_4.message;
                    errorCode = errorCode[0].trim();
                    console.error(tag, "errorCode: ", errorCode);
                    console.error(tag, "errorMessage: ", errorMessage.toString());
                    if (errorCode === '401') {
                        throw Error("102: Missconfiguration! invalid api secret!");
                    }
                    else if (errorCode === '500' && errorMessage.indexOf("Client Callback Error: Posting transaction slate") >= 0) {
                        throw Error("102: invalid slate!");
                    }
                    else {
                        console.error('e', e_4);
                        throw Error("999: Unknown Error: " + e_4.message);
                    }
                    return [3 /*break*/, 4];
                case 4: return [2 /*return*/];
            }
        });
    });
};
