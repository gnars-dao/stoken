"use strict";
/*
    TX builder
        Normalizing tx building

 */
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g;
    return g = { next: verb(0), "throw": verb(1), "return": verb(2) }, typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (_) try {
            if (f = 1, y && (t = op[0] & 2 ? y["return"] : op[0] ? y["throw"] || ((t = y["return"]) && t.call(y), 0) : y.next) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [op[0] & 2, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
Object.defineProperty(exports, "__esModule", { value: true });
var TAG = " | terra-tx-builder | ";
var terra_js_1 = require("@terra-money/terra.js");
var log = require('@pioneer-platform/loggerdog')();
var terra_js_2 = require("@terra-money/terra.js");
module.exports = {
    signTx: function (to, from, amount, memo, seed) {
        return sign_transaction(to, from, amount, memo, seed);
    }
};
/**********************************
 // Lib
 //**********************************/
var sign_transaction = function (to, from, amount, memo, seed) {
    return __awaiter(this, void 0, void 0, function () {
        var tag, mk, terra, wallet, send, sequence, accountNumber, chainId, gas, fee, tx, signed, e_1;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    tag = TAG + " | sign_transaction | ";
                    _a.label = 1;
                case 1:
                    _a.trys.push([1, 3, , 4]);
                    mk = new terra_js_1.MnemonicKey({
                        mnemonic: seed,
                    });
                    terra = new terra_js_1.LCDClient({
                        URL: 'https://fcd.terra.dev',
                        // URL: '',
                        chainID: 'terra',
                    });
                    wallet = terra.wallet(mk);
                    send = new terra_js_1.MsgSend(from, to, { uluna: amount });
                    sequence = 0;
                    accountNumber = 0;
                    chainId = "terra";
                    gas = "80000";
                    fee = new terra_js_2.StdFee(1000000, { uluna: 1000000000 });
                    tx = new terra_js_2.StdSignMsg(chainId, accountNumber, sequence, fee, [send], memo);
                    log.info(tag, "tx: ", tx);
                    return [4 /*yield*/, mk.signTx(tx)];
                case 2:
                    signed = _a.sent();
                    // let tx = await wallet.createAndSignTx({
                    //     msgs: [send],
                    //     memo: memo,
                    //     account_number:accountNumber,
                    //     sequence:sequence,
                    //     fee,
                    //     // @ts-ignore
                    //     chainId
                    // })
                    // let resultBroadcast = await terra.tx.broadcast(tx)
                    // log.info(tag,"resultBroadcast: ",resultBroadcast)
                    //convert to string
                    //convert back to tx
                    //return tx.toJSON()
                    console.log("signed :", signed.toJSON());
                    return [2 /*return*/, signed];
                case 3:
                    e_1 = _a.sent();
                    throw Error(e_1);
                case 4: return [2 /*return*/];
            }
        });
    });
};
