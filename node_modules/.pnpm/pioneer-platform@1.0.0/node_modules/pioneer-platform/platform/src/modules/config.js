...
export const appDir = path.join(APP.getPath('home'), '.fox', 'apps')
export const meeshApp = path.join(APP.getPath('home'), '.fox', 'apps','meesh')
...
export function initConfig (languageSelected) {
  ...
  try {
    ...
    const resp = mkdirp.sync(foxPath)
...
export function checkConfigs () {
  ...
  const fileFound = !!fs.existsSync(foxConfig)
  if (fileFound) {
