...
&nbsp;
        const signatureBuffer = await sign_with_privkey(signMessage, wallet.privateKey !== undefined ? wallet.privateKey : <span class="branch-1 cbranch-no" title="branch not covered" >wallet)</span>
&nbsp;
...
<span class="cstat-no" title="statement not covered" >        console.error(tag, "e: ", e);</span>
...
const sign_with_privkey = async function(signMessage: string, privateKey: any | { valueOf(): string; } | { [Symbol.toPrimitive](hint: "string"): string; }){
    ...
    try{
        <span class="missing-if-branch" title="if path not taken" >I</span>if(!signMessage) <span class="cstat-no" title="statement not covered" >throw Error("signMessage required!")</span>
&nbsp;
        const signature = (typeof privateKey.sign === "function" ? <span class="branch-0 cbranch-no" title="branch not covered" >privateKey.sign(signMessage) </span>: (() =&gt; {
