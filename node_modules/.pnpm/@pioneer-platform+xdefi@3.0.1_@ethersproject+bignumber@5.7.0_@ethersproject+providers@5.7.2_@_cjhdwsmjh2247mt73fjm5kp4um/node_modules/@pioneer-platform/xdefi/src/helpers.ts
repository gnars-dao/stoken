import type { WalletTxParams } from '@pioneer-platform/types';
import { BaseDecimal, Chain, ChainToChainId, ChainToHexChainId } from '@pioneer-platform/types';
...
export const getWalletMethodsForChain = async ({
  ...
  switch (chain) {
    case Chain.THORChain: {
      const { DEFAULT_GAS_VALUE, ThorchainToolbox } = await import(
        '@pioneer-platform/toolbox-cosmos'
      );
    ...
    case Chain.Cosmos: {
      const { GaiaToolbox } = await import('@pioneer-platform/toolbox-cosmos');
    ...
    case Chain.Avalanche: {
      const {
        ...
        covalentApi,
        ethplorerApi,
      } = await import('@pioneer-platform/toolbox-evm');
      ...
      return prepareNetworkSwitch({
        toolbox: {
          ...
          getBalance: async (address: string) => {
            const { getSignatureAssetFor } = await import('@pioneer-platform/swapkit-entities');
            const { baseAmount } = await import('@pioneer-platform/helpers');
