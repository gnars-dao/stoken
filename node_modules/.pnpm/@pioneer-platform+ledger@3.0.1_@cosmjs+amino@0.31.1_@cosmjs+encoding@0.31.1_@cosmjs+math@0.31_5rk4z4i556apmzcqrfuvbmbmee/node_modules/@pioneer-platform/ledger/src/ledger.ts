...
import type { UTXOBuildTxParams } from '@pioneer-platform/toolbox-utxo';
import type {
  ConnectWalletParams,
  DerivationPathArray,
  TxParams,
...
import { Chain, ChainId, FeeOption, RPCUrl, WalletOption } from '@pioneer-platform/types';
...
const getToolbox = async ({
  ...
  switch (chain) {
    case Chain.Bitcoin: {
      ...
      const { BTCToolbox } = await import('@pioneer-platform/toolbox-utxo');
    ...
    case Chain.BitcoinCash: {
      if (!utxoApiKey) throw new Error('UTXO API key is not defined');
      const { BCHToolbox } = await import('@pioneer-platform/toolbox-utxo');
    ...
    case Chain.Dogecoin: {
      if (!utxoApiKey) throw new Error('UTXO API key is not defined');
      const { DOGEToolbox } = await import('@pioneer-platform/toolbox-utxo');
    ...
    case Chain.Litecoin: {
      if (!utxoApiKey) throw new Error('UTXO API key is not defined');
      const { LTCToolbox } = await import('@pioneer-platform/toolbox-utxo');
    ...
    case Chain.Binance: {
      const { BinanceToolbox } = await import('@pioneer-platform/toolbox-cosmos');
    ...
    case Chain.Cosmos: {
      const { getDenom, GaiaToolbox } = await import('@pioneer-platform/toolbox-cosmos');
    ...
    case Chain.Ethereum: {
      ...
      const { ETHToolbox, getProvider } = await import('@pioneer-platform/toolbox-evm');
    ...
    case Chain.Avalanche: {
      ...
      const { AVAXToolbox, getProvider } = await import('@pioneer-platform/toolbox-evm');
    ...
    case Chain.THORChain: {
      const { ThorchainToolbox } = await import('@pioneer-platform/toolbox-cosmos');
