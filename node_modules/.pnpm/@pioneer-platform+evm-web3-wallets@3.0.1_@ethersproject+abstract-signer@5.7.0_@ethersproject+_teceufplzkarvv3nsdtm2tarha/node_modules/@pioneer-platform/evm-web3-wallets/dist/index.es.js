...
var I = /* @__PURE__ */ ((_) => (_.KEYSTORE = "KEYSTORE", _.XDEFI = "XDEFI", _.METAMASK = "METAMASK", _.COINBASE_WEB = "COINBASE_WEB", _.TREZOR = "TREZOR", _.KEEPKEY = "KEEPKEY", _.TRUSTWALLET_WEB = "TRUSTWALLET_WEB", _.LEDGER = "LEDGER", _.KEPLR = "KEPLR", _.OKX = "OKX", _.BRAVE = "BRAVE", _.WALLETCONNECT = "WALLETCONNECT", _))(I || {});
const T = (_) => {
  switch (_) {
    case I.BRAVE:
  ...
  const O = A.map(async (R) => {
    const { getWeb3WalletMethods: D } = await import("@pioneer-platform/toolbox-evm"), s = new d(T(a), "any");
    await s.send("eth_requestAccounts", []);
    const r = await s.getSigner().getAddress(), c = await D({
      chain: R,
      ethplorerApiKey: n,
      covalentApiKey: t,
      ethereumWindowProvider: T(a)
    });
    _({
      chain: R,
      walletMethods: { ...c, getAddress: () => r },
      wallet: { address: r, balance: [], walletType: a }
    });
  });
