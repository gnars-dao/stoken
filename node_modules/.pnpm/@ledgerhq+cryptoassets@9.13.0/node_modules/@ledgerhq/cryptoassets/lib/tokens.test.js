...
describe("tokens", () => {
    beforeEach(() => {
        (0, tokens_1.__clearAllLists)();
    });
    describe("addTokens", () => {
        it("Should list token to be empty", () => {
            expect((0, tokens_1.listTokens)().length).toBe(0);
        });
        it("Add tokens normally and check if delisted work correclty", () => {
            (0, tokens_1.addTokens)(initMainToken.map(tokens_1.convertERC20));
            expect((0, tokens_1.listTokens)().length).toBe(2);
            expect((0, tokens_1.listTokens)({ withDelisted: true }).length).toBe(4);
            expect((0, tokens_1.listTokensForCryptoCurrency)(ethereumCurrency).length).toBe(2);
            expect((0, tokens_1.listTokensForCryptoCurrency)(ethereumCurrency, { withDelisted: true }).length).toBe(4);
        });
        it("Add token and readd the same token shouldn't delete anything", () => {
            ...
            const tokenHash = (0, tokens_1.createTokenHash)((0, tokens_1.convertERC20)(changeToken[0]));
            const existingToken = (0, tokens_1.listTokens)().find(t => t.ticker === "KIBA");
            if (!existingToken)
                throw new Error("Should not be empty");
            expect(tokenHash).toBe((0, tokens_1.createTokenHash)(existingToken));
            ...
            if (!tokenAfterChange)
                throw new Error("Should not be empty");
            expect(tokenHash).toBe((0, tokens_1.createTokenHash)(tokenAfterChange));
        });
        it("Add tokens normally and then delist one token", () => {
            ...
            (0, tokens_1.addTokens)(changeToken.map(tokens_1.convertERC20));
            expect((0, tokens_1.listTokens)().length).toBe(1);
            (0, tokens_1.findTokenById)("kiba_inu");
            expect((0, tokens_1.listTokens)({ withDelisted: true }).length).toBe(4);
            expect((0, tokens_1.listTokensForCryptoCurrency)(ethereumCurrency).length).toBe(1);
            expect((0, tokens_1.listTokensForCryptoCurrency)(ethereumCurrency, { withDelisted: true }).length).toBe(4);
        });
    });
});
