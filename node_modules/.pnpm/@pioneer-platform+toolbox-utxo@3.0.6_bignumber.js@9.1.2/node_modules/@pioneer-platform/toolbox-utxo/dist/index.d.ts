...
export declare const accumulative: ({
  inputs,
  outputs,
  feeRate,
  chain,
  ...
  | {
      inputs: (UTXO | UTXOInputWithScriptType)[];
      outputs: TargetOutput[];
      fee: number;
    }
  | {
...
export declare const BaseUTXOToolbox: (
  baseToolboxParams: UTXOBaseToolboxParams & {
  ...
  apiClient: {
    ...
    scanUTXOs: (params: ScanUTXOsParams) => Promise<
      {
        address: string;
        hash: string;
        index: number;
  ...
  createKeysForPath: (params: UTXOCreateKeyParams) => Promise<ECPairInterface>;
  ...
  getInputsOutputsFee: (params: UTXOBuildTxParams) => Promise<
    | {
        inputs: (UTXO | UTXOInputWithScriptType)[];
        outputs: TargetOutput[];
        fee: number;
  ...
  >;
  getFeeForTransaction: (params: UTXOBuildTxParams) => Promise<AmountWithBaseDenom>;
  estimateMaxSendableAmount: (params: UTXOEstimateFeeParams) => Promise<AmountWithBaseDenom>;
};
...
export declare const BCHToolbox: ({
  apiKey,
  rpcUrl,
  apiClient: client,
}: {
...
export declare interface BlockchairOutputsResponse
...
export declare interface BlockchairTransactionInputOutput
  extends BlockchairSpendingBlockData,
    BlockchairInputOutputCommonData {
  scripthash_type: null | string;
}
...
export declare const BTCToolbox: ({
  apiKey,
  rpcUrl,
  apiClient,
}: {
...
export declare const calculateTxSize: ({
  inputs,
  outputs,
  feeRate,
}: UTXOCalculateTxSizeParams) => number;

export declare const compileMemo: (memo: string) => Buffer;

export declare const DOGEToolbox: ({
  apiKey,
  rpcUrl,
  apiClient,
}: {
...
export declare const getDustThreshold: (chain: UTXOChain) => 550 | 5500 | 100000;

export declare const getInputSize: (input: UTXOInputWithScriptType | UTXO) => number;
...
export declare const LTCToolbox: ({
  apiKey,
  rpcUrl,
  apiClient,
}: {
...
export declare type ScanUTXOsParams = {
  address: string;
  fetchTxHex?: boolean;
};
...
export declare type TransferParams = UTXOWalletTransferParams<
  {
    builder: TransactionBuilderType;
    utxos: UTXO[];
  },
...
export declare type UTXOEstimateFeeParams = {
  from: string;
...
export declare type UTXOInputWithScriptType = UTXO & {
  type: UTXOScriptType;
  address: string;
};
...
export declare type UTXOToolbox = ReturnType<
