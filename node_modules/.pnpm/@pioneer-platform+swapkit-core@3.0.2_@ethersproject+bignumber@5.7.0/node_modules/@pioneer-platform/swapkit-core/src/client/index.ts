...
} from '@pioneer-platform/toolbox-cosmos';
import type {
  AVAXToolbox,
  BSCToolbox,
  ETHToolbox,
...
import type { UTXOEstimateFeeParams, UTXOToolbox } from '@pioneer-platform/toolbox-utxo';
import type {
...
export class SwapKitCore<T = ''> {
  ...
  swap = async ({ streamSwap, recipient, route, feeOptionKey }: SwapParams) => {
    ...
    try {
      ...
      if (SWAP_IN.includes(quoteMode)) {
        ...
        const { getProvider, toChecksumAddress } = await import('@pioneer-platform/toolbox-evm');
  ...
  estimateMaxSendableAmount = async ({
    ...
    switch (chain) {
      ...
      case Chain.Optimism:
      case Chain.Polygon: {
        const { estimateMaxSendableAmount } = await import('@pioneer-platform/toolbox-evm');
      ...
      case Chain.THORChain:
      case Chain.Cosmos: {
        const { estimateMaxSendableAmount } = await import('@pioneer-platform/toolbox-cosmos');
  ...
  private _approve = async <T = string>(
    ...
    const { getTokenAddress } = await import('@pioneer-platform/toolbox-evm');
