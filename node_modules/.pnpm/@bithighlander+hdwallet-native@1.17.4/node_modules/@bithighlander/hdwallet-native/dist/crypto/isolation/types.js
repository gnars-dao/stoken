...
exports.integer = funtypes_1.Number.withConstraint(x => Number.isSafeInteger(x) || `expected ${x} to be an integer`, { name: "Integer" });
...
const uint16 = funtypes_1.Number.withConstraint(x => x === ((x & 0xffff) >>> 0) || `expected ${x} to be an unsigned 16-bit integer`, { name: "Uint16" });
exports.Uint16 = uint16;
const uint8 = funtypes_1.Number.withConstraint(x => x === ((x & 0xff) >>> 0) || `expected ${x} to be an unsigned 8-bit integer`, { name: "Uint8" });
exports.Uint8 = uint8;
function boundedUintBase(max) {
    return Object.assign(exports.Uint.withConstraint(x => x <= max || `expected ${x} to be less than or equal to ${max}`, { name: `BoundedUint(${max})` }), { max });
}
...
function boundedStringBase(regex) {
    return Object.assign(funtypes_1.String.withConstraint(x => regex.test(x) || `expected ${x} to match regex ${regex}`, { name: `BoundedString(${regex})` }), { regex });
}
