...
import { BIP_32_PATH_REGEX, BIP_39_PATH_REGEX, MIN_BIP_44_DEPTH, SLIP_10_PATH_REGEX } from './constants';
 ...
 */ export async function deriveKeyFromPath(args) {
    ...
    if (node && !(node instanceof SLIP10Node) && !(node instanceof BIP44Node) && !(node instanceof BIP44CoinTypeNode)) {
 ...
 */ export function validatePathSegment(path, hasKey, depth) {
    ...
    path.forEach((node, index)=>{
        if (index === 0) {
            startsWithBip39 = node instanceof Uint8Array || BIP_39_PATH_REGEX.test(node);
            if (// TypeScript is unable to infer that `node` is a string here, so we
            // need to explicitly check it again.
            !(node instanceof Uint8Array) && !startsWithBip39 && !BIP_32_PATH_REGEX.test(node) && !SLIP_10_PATH_REGEX.test(node)) {
                throw getMalformedError();
            }
        } else if (node instanceof Uint8Array || !BIP_32_PATH_REGEX.test(node) && !SLIP_10_PATH_REGEX.test(node)) {
            throw getMalformedError();
        }
    });
