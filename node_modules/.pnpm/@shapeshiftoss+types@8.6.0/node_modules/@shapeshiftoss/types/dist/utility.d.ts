...
export declare type ValueOfUnion<T, K> = T extends unknown ? (K extends keyof T ? T[K] : undefined) : never;
/** Get all keys in union(T) as union */
export declare type KeysOfUnion<T> = T extends unknown ? keyof T : never;
/**
 ...
 */
export declare type UnionMapping<T> = {
    [K in KeysOfUnion<T> | keyof T]: ValueOfUnion<T, K>;
};
/** Pick out the elements we know for sure and make everything else optional */
export declare type UnionMerge<T> = Pick<UnionMapping<T>, keyof T> & Partial<UnionMapping<T>>;
/** Adds all possible values of union(T) as a nested object under a `chainSpecific` key */
export declare type ChainSpecific<T, M> = UnionMerge<T extends unknown ? (T extends keyof M ? {
    chainSpecific: M[T];
} : undefined) : never>;
export declare type PartialRecord<K extends keyof any, V> = Partial<Record<K, V>>;
