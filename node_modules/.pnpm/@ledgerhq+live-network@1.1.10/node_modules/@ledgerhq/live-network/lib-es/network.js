...
export const responseInterceptor = (response) => {
    ...
    log("network-success", `${response.status} ${method} ${baseURL || ""}${url} (${(Date.now() - startTime).toFixed(0)}ms)`, { data: response.data });
    return response;
};
export const errorInterceptor = (error) => {
    ...
    if (error.response) {
        ...
        log("network-error", `${status} ${method} ${baseURL || ""}${url} (${duration}): ${errorToThrow.message}`, getEnv("DEBUG_HTTP_RESPONSE") ? { data: data } : {});
        throw errorToThrow;
    }
