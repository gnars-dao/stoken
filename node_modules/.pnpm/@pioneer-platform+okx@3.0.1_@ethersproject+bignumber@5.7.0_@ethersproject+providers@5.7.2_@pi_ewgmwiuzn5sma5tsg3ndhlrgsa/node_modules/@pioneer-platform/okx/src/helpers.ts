...
import type { getWeb3WalletMethods } from '@pioneer-platform/toolbox-evm';
import type { BTCToolbox, UTXOTransferParams } from '@pioneer-platform/toolbox-utxo';
...
export const cosmosTransfer =
  ...
  async ({ from, recipient, amount, asset, memo }: any) => {
    ...
    const { createCosmJS } = await import('@pioneer-platform/toolbox-cosmos');
...
> => {
  switch (chain) {
    ...
    case Chain.BinanceSmartChain: {
      ...
      const { getWeb3WalletMethods, getProvider } = await import('@pioneer-platform/toolbox-evm');
